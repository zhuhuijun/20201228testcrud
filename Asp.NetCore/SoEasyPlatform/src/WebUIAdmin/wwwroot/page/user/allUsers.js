layui.config({
    ...
    var MyCrudHelper = (function () {
                ...
                $.ajax({
                    ...
                    success: function (e) {
                        ...
                        if (e.code === 200) {
                            ...
                            ColHelper.ErrorMsg(e.msg, "error");
                        }
                    },
                    error: function () {
        ...
        Crud.NavAssignRole = function () {
            ...
            if ($checkbox.is(":checked")) {
                ...
                var rowid = $($checked[0]).attr("data-id");
                Crud.ShowAssignRoleUI(rowid);
            } else {
                layer.msg("请选择需要授权的用户!");
            }
        };
        /**
         * 为角色分配用户
         * @param {any} rowid
         */
        Crud.ShowAssignRoleUI = function (rowid) {
            var index = layui.layer.open({
                title: Crud.ConstField.AssignRoleTitle,
                type: 2,
                area: ['500px', '400px'],
                content: Crud.ConstField.AssignRole + rowid,
     ...
     * @param {any} options
     */
    window.laytoaster = function (message, type) {
    ...
    /*为用户授权相应的角色*/
    $(".batchRole").click(MyCrudHelper.NavAssignRole);
    //全选
        ...
        if (currData.rows.length !== 0) {
            ...
            for (var i = 0; i < dataArr.length; i++) {
                dataHtml += '<tr>'
                    ...
                    + '<td>' + dataArr[i].DisplayName + '</td>'
                    + '<td>' + dataArr[i].Description + '</td>'
                    + '<td>' + dataArr[i].ApprovedBy + '</td>'
                    + '<td>' + dataArr[i].RegisterTime + '</td>'
                    + '<td>'
